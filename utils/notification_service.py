...
if __name__ == "__main__":
    ...
    # triggered via `issue_comment` for CI on pull requests (using comment `run-slow`)
    # TODO: remove the `push` part before merge
    elif os.environ.get("GITHUB_EVENT_NAME") in ["issue_comment", "pull_request"]:
        # The id of the workflow `.github/workflows/self-scheduled-caller.yml` (not of a workflow run of it).
        # TODO: Make this flexible once we implement `run-slow` for AMD CI and others.
        prev_workflow_id = "90575235"
        # TODO: Find the last completed scheduled workflow run with the commit being a parent of the event commit.
        prev_workflow_run_id = get_last_daily_ci_workflow_run_id(
